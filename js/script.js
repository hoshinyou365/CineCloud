...
async function getMovies(){
  ...
  return data.movies.map(m=>({
    ...
    image:(m.image_url || "images/cinema.svg").replace(/\.jpg$/i,".svg")
  }));
}
    ...
    grid.innerHTML=movies.map(m=>`
      <article class="movie-card">
        <img src="${escapeHtml(m.image)}" alt="${escapeHtml(m.title)}"
             onerror="this.onerror=null;this.src='images/cinema.svg'">
...
async function setupBooking(){
  ...
  if(!form)return;
  if(!sessionStorage.getItem("cinecloudUser")){
    location.href="login.html?next=booking.html";
    return;
  }
